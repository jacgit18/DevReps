function insertApproachOne(intervals, newInterval) {
  ...
  while (i < intervals.length && intervals[i][1] < newInterval[0]) {
    //intervals for far left
    result.push(intervals[i])
    i++
  }

  while (i < intervals.length && intervals[i][0] <= newInterval[1]) {
    //possible overlap and newInterval
...
function insertApproachTwo(intervals, newInterval) {
  ...
  for (let i = 0; i < intervals.length; i++) {
    if (newInterval[1] < intervals[i][0]) {
      //if the newInterval just doesnt over lap and can be added to far left
      result.push(newInterval)
      ...
      //if there is no overlap between two, and
      result.push(intervals[i])
    } else {
