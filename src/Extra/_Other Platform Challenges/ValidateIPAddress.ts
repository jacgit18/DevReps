...
function validateIP(ip: string): boolean {
  ...
  //nst set = new Set(ip)
  //nsole.log(set)
  // Number of periods: 3, and each number before and after the periods should be less than or equal to 3
  let count = 0
  ...
  for (let current = 0; current < ip.length; ++current) {
    ...
    // Check if the index value is greater than 3 when we encounter a period
    if (count > 0 && current > 3) {
      return false
    }
